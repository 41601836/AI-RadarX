// WAD模型核心优化测试
import { calculateDecayWeight, calculateBatchDecayWeights } from './lib/algorithms/wad';
...
function testDecayWeightOptimization() {
  ...
  // 测试当前实现
  const currentResult = calculateDecayWeight(testTimestamp, now, 0.1);
  console.log(`当前实现结果: ${currentResult}`);
  
  // 测试优化实现
  function optimizedCalculateDecayWeight(timestamp: number, currentTime: number, decayRate: number = 0.1): number {
  ...
  for (const ts of timestamps) {
    calculateDecayWeight(ts, now, 0.1);
  }
