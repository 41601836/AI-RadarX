...
exports.getHHILevel = getHHILevel;
exports.calculateGiniCoefficient = calculateGiniCoefficient;
exports.getGiniLevel = getGiniLevel;
exports.calculateCRIndicator = calculateCRIndicator;
exports.calculateChipConcentration = calculateChipConcentration;
...
exports.calculateChipTrend = calculateChipTrend;
exports.calculateEnhancedChipDistribution = calculateEnhancedChipDistribution;
exports.calculateCompositeChipStrength = calculateCompositeChipStrength;
// 筹码分布算法实现
...
// 计算基尼系数（高精度实现）
function calculateGiniCoefficient(chipData) {
    if (chipData.length === 0)
        return 0;
    // 按价格排序
...
function calculateCRIndicator(params) {
...
// 二分查找价格索引（用于提高性能）
function binarySearchPriceIndex(data, targetPrice) {
    let left = 0;
...
function calculateChipConcentration(params) {
    ...
    const concentrationScore = parseFloat(((getHHILevelWeight(hhiLevel) * 0.4) +
        (getGiniLevelWeight(giniLevel) * 0.3) +
        (crIndicator.strength * 0.3)).toFixed(2));
    // 确定综合集中度等级
    const concentrationGrade = getConcentrationGrade(concentrationScore);
    // 生成综合解读
    const interpretation = generateConcentrationInterpretation(concentrationGrade, hhiLevel, giniLevel, crIndicator);
    return {
        hhi,
        hhiLevel,
        gini,
...
// 辅助函数：获取HHI等级权重
function getHHILevelWeight(level) {
    switch (level) {
...
// 辅助函数：获取基尼系数等级权重
function getGiniLevelWeight(level) {
    switch (level) {
        case 'low': return 0.25;
        case 'medium': return 0.5;
        case 'high': return 0.75;
...
// 辅助函数：获取综合集中度等级
function getConcentrationGrade(score) {
    if (score < 0.4)
        return 'low';
    if (score < 0.6)
        return 'medium';
...
// 辅助函数：生成综合解读
function generateConcentrationInterpretation(grade, hhiLevel, giniLevel, crIndicator) {
    const gradeInterpretations = {
        'low': '筹码高度分散',
        'medium': '筹码中等集中',
        'high': '筹码高度集中',
        'very_high': '筹码极度集中'
...
// 计算平均成本
function calculateAverageCost(chipData) {
    if (chipData.length === 0)
        return 0;
    // 合并为单个循环，减少迭代次数
    let totalValue = 0;
...
function identifyChipPeaks(chipData, isSorted = false, qualityParams = {}) {
    const { minVolumeThreshold = 0.05, // 最小成交量阈值（占总成交量的百分比）
    ...
    if (chipData.length === 0) {
        const emptyPeak = { price: 0, ratio: 0, volume: 0, width: 0, dominance: 0, strength: 0, reliability: 0, centerPrice: 0, volumeWeightedPrice: 0 };
        return {
            peakPrice: 0,
            peakRatio: 0,
            isSinglePeak: true,
    ...
    for (let i = 1; i < gradients.length; i++) {
        if (gradients[i - 1] > 0 && gradients[i] < 0) {
            ...
            // 计算峰值可靠性
            const reliability = calculatePeakReliability(sortedData, peakIndex, leftIndex, rightIndex, dominance, strength);
            // 应用质量阈值过滤
...
// 计算峰值可靠性
function calculatePeakReliability(sortedData, peakIndex, leftIndex, rightIndex, dominance, strength) {
    ...
    // 计算可靠性（0-1）
    const reliability = (dominance * 0.4 +
        strength * 0.3 +
        symmetry * 0.2 +
...
// 合并接近的峰值
function mergeClosePeaks(peaks, sortedData, mergeDistancePercentage, useVolumeWeighting) {
    if (peaks.length <= 1)
        return peaks;
    // 按价格排序峰值
    const sortedPeaks = [...peaks].sort((a, b) => a.price - b.price);
    const mergedPeaks = [];
    let currentMergeGroup = [sortedPeaks[0]];
...
// 合并多个峰值为单个峰值
function combinePeaks(peaks, sortedData, useVolumeWeighting) {
    // 按强度排序，确定主要峰值
...
// 计算峰值质量综合评分
function calculatePeakQualityScore(peaks, dominantPeak, sortedData) {
    if (peaks.length === 0)
        return 0;
    // 计算总成交量
    const totalVolume = sortedData.reduce((sum, item) => sum + item.volume, 0);
    ...
    // 计算峰值质量综合评分
    const qualityScore = (dominantPeak.strength * 0.4 +
...
function calculateSupportResistance(chipData, currentPrice, isSorted = false) {
    ...
    // 检测价格簇（连续的密集区）
    const detectPriceClusters = (startIndex, endIndex, direction) => {
        const clusters = [];
...
function calculateChipTrend(chipDataHistory) {
...
function calculateEnhancedChipDistribution(priceData, options = {}) {
    const { wadOptions = { decayRate: 0.1, weightType: 'volume' }, wadWeightFactor = 0.3, volumeWeightFactor = 0.7 } = options;
    ...
    // 1. 计算WAD指标
    const wadData = (0, wad_1.calculateCumulativeWAD)(priceData, wadOptions);
    // 2. 计算基础筹码分布
    ...
    // 创建价格区间
    const baseChipDistribution = new Array(numPriceIntervals);
    ...
    // 预分配增强筹码数据数组
    const enhancedChipData = new Array(numPriceIntervals);
    // 构建价格到WAD索引的映射，避免重复查找
    const priceToWadIndexMap = new Map();
...
function calculateCompositeChipStrength(enhancedChipData) {
