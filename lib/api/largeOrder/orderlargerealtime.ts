...
// 定义参数接口
export interface OrderLargeRealTimeParams {
  stockCode?: string;
...
// Mock数据生成器
export const generateOrderLargeRealTimeMock: MockDataGenerator<LargeOrderRealTimeData> = async (params: OrderLargeRealTimeParams) => {
...
export async function fetchOrderLargeRealTime(
  params: OrderLargeRealTimeParams
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_API_MOCK === 'true') {
      ...
      dataSource = 'Mock';
      return apiGet<LargeOrderRealTimeData>(
        '/order/large/real-time',
        params,
        { requiresAuth: false },
        generateOrderLargeRealTimeMock
      );
    }
    ...
    try {
      ...
      const response = await apiGet<LargeOrderRealTimeData>(
        '/order/large/real-time',
        params,
        { requiresAuth: false }
      );
  ...
  const mockResponse = await apiGet<LargeOrderRealTimeData>(
    '/order/large/real-time',
    params,
    { requiresAuth: false },
    generateOrderLargeRealTimeMock
