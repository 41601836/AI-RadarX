...
// 定义参数接口
export interface OrderLargeTrendParams {
  stockCode?: string;
  timeType?: string;
  days?: number;
}
...
// Mock数据生成器
export const generateOrderLargeTrendMock: MockDataGenerator<LargeOrderTrendData> = async (params: OrderLargeTrendParams) => {
...
export async function fetchOrderLargeTrend(
  params: OrderLargeTrendParams
  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_API_MOCK === 'true') {
      ...
      dataSource = 'Mock';
      return apiGet<LargeOrderTrendData>(
        '/order/large/trend',
        params,
        { requiresAuth: false },
        generateOrderLargeTrendMock
      );
    }
    ...
    try {
      ...
      const response = await apiGet<LargeOrderTrendData>(
        '/order/large/trend',
        params,
        { requiresAuth: false }
      );
  ...
  const mockResponse = await apiGet<LargeOrderTrendData>(
    '/order/large/trend',
    params,
    { requiresAuth: false },
    generateOrderLargeTrendMock
